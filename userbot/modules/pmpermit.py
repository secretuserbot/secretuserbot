...
async def permitpm(event):
    """ Sizə icazəsi  olmadan PM göndərən insanları qadağan etmək üçündür. \
        Yazmağa davam edən istifadəçiləri bloklayır. """
    if PM_AUTO_BAN:
...
async def auto_accept(event):
    """ İlk mesaj yazan sizsinizsə, bu avtomatik olaraq təsdiqlənir. """
    if not PM_AUTO_BAN:
        return
    self_user = await event.client.get_me()
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
...
async def notifoff(noff_event):
    """ .notifoff əmri təsdiqlənməmiş şəxslərdən PM-lərdən bildiriş almamağa imkan verir. """
    try:
        from userbot.modules.sql_helper.globals import addgvar
    except AttributeError:
        await noff_event.edit("`Qeyri-SQL rejimində çalışan bot!`")
        return
...
async def notifon(non_event):
    """ .notifon əmri təsdiqlənməmiş PM-lərdən bildirişlər almağa imkan verir. """
    try:
        from userbot.modules.sql_helper.globals import delgvar
    except:
        await non_event.edit("`Qeyri-SQL rejimində çalışan bot!`")
        return
    delgvar("NOTIF_OFF")
...
async def approvepm(apprvpm):
    """ .approve əmri hər kəsə PM göndərməyə imkan verir. """
    try:
        from userbot.modules.sql_helper.pm_permit_sql import approve
    except:
        await apprvpm.edit("`Qeyri-SQL rejimində çalışan bot!`")
        return
...
async def disapprovepm(disapprvpm):
    try:
        ...
        await disapprvpm.edit("`Qeyri-SQL rejimində çalışan bot!`")
        return
