...
@register(outgoing=True, pattern="^.deyi[sş]dir ?(.*)")
async def degistir(event):
    ...
    plugin = plugin.strip()
    TURLER = ["afk", "alive", "pm", "kickme", "ban", "mute", "approve", "disapprove", "block"]
    if type(mesaj) == list:
        if plugin in TURLER:
            if event.is_reply:
            ...
            await event.edit(LANG['NOT_FOUND'] + ":`afk/alive/pm/kickme/ban/mute/approve/disapprove/block`")
    elif len(plugin) < 1:
...
CmdHelp('deyisdir').add_command(
    'deyişdir', '<module> <message/response>', 'Əvəz et, botdakı plagin mesajlarını dəyişməyə imkan verir. Mesaj yazmasanız, Plugin orijinal mesajı qaytaracaq.', '.deyisdir afk \"Hazırda burada deyiləm... Bəlkə də heç gəlmədim"'
).add_info(
    '**Dəstəklənən Pluginlər:** `afk/alive/pm/kickme/ban/mute/approve/disapprove/block`\n**Alive Dəyişənləri:** `{plugin}, {telethon}, {secret}, {python}, {username}, {fullname}, {lastname}, {id}, {mention}`\n\
...
**PMpermit Dəyişənləri(pm, block, approve, disapprove):** `{id}, {username}, {mention}, {first_name}, {last_name}`\
**Kickme Dəyişənləri:** `{title}`'
).add()
