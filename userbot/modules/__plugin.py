    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Modul uğurla yükləndi!**\n__Modulun əmrləri və istifadəsi haqqında məlumat üçün __ `.secret {cmdhelp}` __.__ yazın.')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Modul uğurla yükləndi!**\n__Modulun əmrləri və istifadəsi haqqında məlumat əldə etmək üçün __ `.secret {fileName}` yazın.__')
...
async def ptest(event):
    ...
    return await event.edit(f'**yükləndi!**\
    \n__Modulunuzu sınaqdan keçirə bilərsiniz. Botu yenidən başlatdığınız zaman plugin işləməyəcək.')
