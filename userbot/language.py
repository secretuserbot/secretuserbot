...
LOGS.info("Dil Faylı Endirilir...")
LANGUAGE_JSON = None

for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "secretjson")):
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.secretjson"):
                    LOGS.warn("Varsayılan dil faylı istifadə edilir...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.secretjson", "r").read())
                else:
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.secretjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.secretjson", "r").read())
        except JSONDecodeError:
